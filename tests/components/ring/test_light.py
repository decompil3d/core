...
async def test_entity_registry(hass, requests_mock):
    ...
    entry = entity_registry.async_get("light.landscape")
    assert entry.unique_id == "mock-group-id"
...
async def test_light_group_off_reports_correctly(hass, requests_mock):
    """Tests that the initial state of a light group that should be off is correct."""
    await setup_platform(hass, LIGHT_DOMAIN)

    state = hass.states.get("light.landscape")
    assert state.state == "off"
    assert state.attributes.get("friendly_name") == "Landscape"
...
async def test_light_can_be_turned_on(hass, requests_mock):
    ...
    requests_mock.put(
        ...
        text=load_fixture("ring/doorbot_siren_on_response.json"),
    )
...
async def test_light_group_can_be_turned_on_and_off(hass, requests_mock):
    ...
    await hass.services.async_call(
        "light", "turn_on", {"entity_id": "light.landscape"}, blocking=True
    )
    await hass.async_block_till_done()
...
async def test_updates_work(hass, requests_mock):
    ...
    requests_mock.get(
        ...
        text=load_fixture("ring/devices_updated.json"),
    )
...
async def test_group_updates_work(hass, requests_mock):
    ...
    requests_mock.get(
        "https://api.ring.com/groups/v1/locations/mock-location-id/groups/mock-group-id/devices",
        text=load_fixture("ring/group_devices_updated.json"),
    )
